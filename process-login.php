...
if (isset($_POST['email_account']) && isset($_POST['password_account'])) {
	...
	if (mysqli_num_rows($call_back_check_account) == 1) {
		...
		if ($result_check_account['lock_account']) {
			echo '<h1>บัญชีนี้ถูกระงับชั่วคราว</h1>';
			echo "<h2>ระงับบัญชีนี้เป็นเวลา $time_ban_account นาที เพราะผู้ใช้กรอกรหัสผ่านผิด $count  ครั้ง</h2>";
			echo "<h2>บัญชีนี้จะถูกปลดจาการระงับเมื่อถึงเวลา $ban </h2>";
			echo '<a href="form-login.php">กลับไปหน้าเข้าสู่ระบบ</a>';
			...
			die(header('Location: form-login.php')); //รหัสผ่านผิด
		}
	} else {
		die(header('Location: form-login.php')); //ไม่มีอีเมลนี้ในระบบ
	}
} else {
	die(header('Location: form-login.php')); //ไม่ได้กรอกข้อมูล
